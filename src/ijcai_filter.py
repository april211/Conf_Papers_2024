...
def load_data():
    data = pd.read_csv("data/IJCAI_2024.csv", 
                                dtype={"paper_title": "str", "abstract": "str"})
    return data
...
if __name__ == "__main__":
    ...
    data["text"] = data["paper_title"] + " " + data["abstract"]
    ...
    data["paper_title"] = data["paper_title"].str.replace(r'\s+', ' ', regex=True)
    ...
    cnt = 0
    with open("res/raw/ijcai/ijcai_results.md", "w") as f:
        for row in data.itertuples():
            ...
            if pos_score != 0 and neg_score == 0:
                cnt += 1
                f.write(data.loc[row.Index]["paper_title"] + "\n")
                ...
                f.write(data.loc[row.Index]['abstract'] + "\n\n")
            # else:
            #     if data.loc[row.Index]["paper_title"] == "Improving Equivariant Graph Neural Networks on Large Geometric Graphs via Virtual Nodes Learning":
